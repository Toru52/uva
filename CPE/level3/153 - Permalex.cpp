	...
	while (scanf("%s", s),s[0]!='#') {
		...
		for (int i = 0; s[i]; i++) {
			for (int j = 0; j < s[i] - 'a'; j++) {
				//ab->continue ba->go down(j=0(a))
				if (cnt[j] == 0) 
					continue;
				//printf("i %d j %d cnt %c\n",i,j, s[i]);
				//when use a alpha 'cnt[j]',being this to a stop temporary 
				cnt[j]--;
				int set[50] = {};
				// create (k-1)!
				...
				long long tmp = 1;
				for (int k = 2; k < len - i; k++)
					tmp *= set[k];//, printf("tmp %lld set[%d]=%d \n", tmp,k,set[k]);
				ans += tmp;
				//back to the original 
				cnt[j]++;
			}
			//elminated used alpha
			cnt[s[i] - 'a']--;
		}
		printf("%10lld\n", ans + 1);
	}
